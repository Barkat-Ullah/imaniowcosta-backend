...
const createCareGiverInDb = async (req: Request) => {
  ...
  if (result.role === UserRoleEnum.CARE_GIVER) {
    const invitationEmail = inviteUserEmail(
      payload.fullName,
      payload.email,
      payload.password,
    );
    await emailSender(payload.email, invitationEmail, 'User Invitation');
  }
